class Angle {
  ...
  add (other) { return new Angle(this.d + other.d) }

  sub (other) { return new Angle(this.d - other.d) }

  mul (other) { return new Angle(this.d * other) }

  lt (other) { return this.d < other.d }
  ...
  standardize (center = 0, period = 360, radian = false) {
    ...
    while (v.lt(l)) v = v.add(p)
    const r = c.add(p.mul(0.5))
    while (r.lt(v)) v = v.sub(p)
    return v
  }
...
class 坐标点 {
  ...
  get 坐标 () { return [this.φ.d, this.λ.d] }

  往 (that) { return new 大圆线段(this, that) }
  ...
  绘图线段 (地图中心 = 120) { return 生成绘图航线(this.范围, 地图中心) }

  get 文字坐标 () { return `${this.φ.dms} ${this.φ.sign('N', 'S')}, ${this.λ.dms} ${this.λ.sign('E', 'W')}` }

  toString () { return this.文字坐标 }

  绘图坐标 (地图中心) { return [this.φ.d, this.λ.standardize(地图中心).d] }
}
...
class 大圆线段 {
  ...
  get 大圆航线 () {
    const array = []
    for (let i = 0; i < 1; i += 10 / this.距离) array.push(this.中间点(i))
    array.push(this.终)
    return array
  }

  绘图线段 (地图中心 = 120) { return 生成绘图航线(this.大圆航线, 地图中心) }
}
